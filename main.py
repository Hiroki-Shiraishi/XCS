...
print("      theta_mna =", parameters.theta_mna)
...
for j in range(parameters.iteration + parameters.condense_iter):
    ...
    this_syserr += system_error(rand_state, my_xcs.exploitation(rand_state),
                                my_xcs.action_prediction(rand_state, my_xcs.exploitation(rand_state)))
...
classifierList[0][10]= "Accuracy"
for clas in my_xcs.population:
    classifierList.append(
        [clas.id, clas.condition, clas.action, clas.fitness, clas.prediction, clas.error, clas.experience,
         clas.time_stamp, clas.action_set_size, clas.numerosity, clas.kappa])
...
print(
    "\nThe whole process is over. After this, please check reward.csv, classifier.csv, and accuracy.csv files in "
    "'result' folder. Thank you.")
